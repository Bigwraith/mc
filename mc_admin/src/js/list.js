    ...
    if(Cookie.get('usname')){
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th class="goods_category">描述</th>
                                <th class="goods_unit">单价</th>
                                <th class="goods_stock">规格</th>
        ...
        function create(str) {
            ...
            <tr name="${str.number}">
                ...
                <td class="goods_id">${str.number}</td>
                <td class="goods_img">
                    <img src="${str.photo}" class="littleimg" alt="" style="width: 70px;height: 90px;border: 1px solid #ccc;">
                    <img src="${str.photo}" class="smallimg" alt="" style="width: 150px;height: 180px;border: 1px solid #ccc;">
                </td>                         
                <td class="goods_name">${str.subject}</td>
                <td class="goods_category">${str.presubject}</td>
                <td class="goods_unit">￥${str.sale_price}</td>
                <td class="goods_stock">${str.formats}</td>
                <td class="goods_del">
                    <i class="iconfont icon-caozuo" title="编辑" name="${str.number}"></i>
                    <i class="iconfont icon-hekriconshanchu" title="删除" name="${str.number}"></i> 
                </td>
                <td class="goods_up">
                    <span class="uplists" name="${str.number}">未上架</span>
                    <span class="downlists" name="${str.number}">下架</span>
                </td>
            </tr>
            `;

            var tbodys = tbody.children; // 所有的tr
            xhr.open('GET', '/groundinglist', true);
            xhr.send();
            xhr.onload = () => {
                if (xhr.status == 200) {
        ...
        function show() {
            xhr.open('GET', '/admin_lists?pages=' + page + '&qtys=' + qtys, true);
            xhr.send();
            xhr.onload = () => {
                if (xhr.status == 200) {
                    // console.log(xhr.responseText);
        ...
        function btn() {
            // 按钮的渲染
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '/admin_listBtn?page=' + page, true);
            xhr.send();
            xhr.onload = () => {
                if (xhr.status == 200) {
                    // console.log(xhr.responseText);
            ...
            for (var i = 0; i < tbodys.length; i++) {
                tbodys[i].addEventListener('click', function (e) {
                    ...
                    if (target.className == 'iconfont icon-caozuo') {
                        ...
                        var number = target.getAttribute('name');
                        xhr.open('GET', '/admin_checklist?number=' + number, true);
                        xhr.send();
                        xhr.onload = () => {
                            if (xhr.status == 200) {
                                var res = JSON.parse(xhr.responseText);
                                ...
                                tbody_edit.innerHTML = `
                                    ...
                                    <td>
                                        <div class="listsimg" style="width:80px;height:100px;border: 1px solid #ccc">
                                            <img src="${data.photo}" class="Gimg" alt="" style="width:80px;height:100px;">
                                        </div>
                                    </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <td>商品名称：</td>
                                    <td>
                                        <input type="text" name="" id="Gname" class="edit_inp" value="${data.subject}">
                                    </td>
                                    </tr>
                                    <tr>
                                    <td>描述：</td>
                                    <td>
                                        <input type="text" name="" id="Gcategory" class="edit_inp" value="${data.presubject}">
                                    </td>
                                    </tr>
                                    <tr>
                                        <td>单价：</td>
                                        <td>
                                            <input type="text" name="" id="Gunit" class="edit_inp" value="${data.sale_price}">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>规格：</td>
                                        <td>
                                            <input type="text" name="" id="Gstock" class="edit_inp" value="${data.formats}">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                ...
                                edit_confirm.onclick = function () {
                                    ...
                                    var url = `/admin_updatelist?number=${number}&imgurl=${encodeURI(imgurl)}&Gname=${encodeURI(Gname.value)}&Gcategory=${encodeURI(Gcategory.value)}&Gunit=${Gunit.value}&Gstock=${Gstock.value}`;
                    ...
                    if (target.className == 'iconfont icon-hekriconshanchu') {
                        ...
                        console.log(number)
                        if (ensure) {
                            xhr.open('GET','/admin_remlist?number='+number, true);
                            xhr.send();
                            xhr.onload = () => {
                                if (xhr.status == 200) {
                                    // console.log(xhr.responseText);
                                    ...
                                    // btn();
                                    if(res.code==1){
                                        xhr.open('GET','/admin_updateid?number='+number,true);
                                        xhr.send();
                                        xhr.onload = ()=>{
                                            if(xhr.status == 200){
                                                // console.log(xhr.responseText);
                            ...
                            xhr.open('GET','/up_remlist?number='+number, true);
                            xhr.send();
                            xhr.onload = () => {
                                if (xhr.status == 200) {
                                    // console.log(xhr.responseText);
                                    ...
                                    // btn();
                                    if(res.code==1){
                                        xhr.open('GET','/up_updateid?number='+number,true);
                                        xhr.send();
                                        xhr.onload = ()=>{
                                            if(xhr.status == 200){
                                                // console.log(xhr.responseText);
                    ...
                    if (target.className == 'uplists') {
                        ...
                        var photo = target.parentNode.parentNode.children[2].children[0].getAttribute('src');
                        var subject = target.parentNode.parentNode.children[3].innerHTML;
                        var presubject = target.parentNode.parentNode.children[4].innerHTML;
                        var sale_price = target.parentNode.parentNode.children[5].innerHTML.slice(1);
                        var formats = target.parentNode.parentNode.children[6].innerHTML
                        // console.log(formats)
                        var url = `/uplists?number=${encodeURI(number)}&photo=${encodeURI(photo)}&subject=${encodeURI(subject)}&presubject=${encodeURI(presubject)}&sale_price=${encodeURI(sale_price)}&formats=${encodeURI(formats)}`;
                        xhr.open('GET', url, true);
                        xhr.send();
                        xhr.onload = () => {
                        if (xhr.status == 200) {
                    ...
                    if (target.className == 'downlists') {
                        var number = target.getAttribute('name');
                        xhr.open('GET', '/downlists?number='+number, true);
                        xhr.send();
                        xhr.onload = () => {
                            // console.log(xhr.responseText);
                            var arr = JSON.parse(xhr.responseText);
